...
3. I wrap each api response with dataerror wrapper in order to give the uses more information in case of failures  

### possible improvements:
1. I use a custom dependency injection with modified cake-pattern. Once the project grows it would become unbearable 
to build all the beans by ourselves. An annotation-based dependency injection framework would be useful.
...
3. I could add many more exceptions depending on the twitter response or just return twitter error codes and messages
  to the user
4. Shouting could be improved, for example, the links inside tweets should not be upper-cased. The instruction does 
